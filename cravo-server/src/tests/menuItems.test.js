...
beforeEach(async () => {
  ...
  await User.findOneAndUpdate({ email: 'admin@example.com' }, { isAdmin: true })
...
describe('Admin — menu items', () => {
  it('creates a menu item', async () => {
    const res = await request(app)
      ...
      .send({ restaurantId, name: 'Zinger Burger', category: 'Burgers', price: 650 })
  ...
  it('rejects a menu item with a negative price', async () => {
    const res = await request(app)
      .post('/api/admin/menu-items')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ restaurantId, name: 'Broken Item', category: 'Burgers', price: -50 })
    expect(res.status).toBe(400)
  })

  it('updates a menu item', async () => {
    const item = await MenuItem.create({
      restaurantId, name: 'Chicken Broast', category: 'Fast Food', price: 500,
    })
  ...
  it('deletes a menu item', async () => {
    const item = await MenuItem.create({
      restaurantId, name: 'Fries', category: 'Sides', price: 200,
    })
